        ...
        private void OnDrawGizmos()
        {
            ...
            for (int x = 0; x < tilemapSize.x; x++)
            {
                for (int y = 0; y < tilemapSize.y; y++)
                {
                    ...
                    Vector3Int cellPosition = tilemapOrigin + new Vector3Int(x, y);
                    ...
                    float3 worldPosition =
                        worldOrigin + new float3(x * cellSize.x, y * cellSize.y, 0);

                    // Quickly check camera point in view frustrum
                    if (!FrustumUtil.IsPointInFrustum(worldPosition, frustumPlanes)) continue;
